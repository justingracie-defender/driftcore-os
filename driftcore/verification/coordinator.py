...
class VerificationCoordinator:
    def __init__(self, guard: InvariantGuard, classifier,
        ...
        if self._egress_targets:
            from driftcore.authority.egress_authorization import StandingEgressPolicy
            # This is the existing operator-only construction surface. Copy the
            # policy into this coordinator's guard, never mutate a shared guard
            # or read runtime owner/target permission booleans.
            self.guard = guard.with_egress_authorizer(StandingEgressPolicy(
                self._egress_targets, egress_owner))
    ...
    def _authorization_for(self, request, effects: set) -> ActionContext:
        ...
        if isinstance(request, dict):
            ...
            if isinstance(raw, str):
                ...
                targets = tuple(raw) if all(type(t) is str for t in raw) else ()
