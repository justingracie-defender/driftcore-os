...
class HumanApprovalGate:
    ...
    Required by ProductionActuationBroker; opt-in for the development base broker.

    CLAIM installed-gate-requires-approval: Once installed, every actuation must
    present an attestation; there is no per-request opt-out.
    """

    def __init__(self, verifier: HumanIdentityVerifier, *,
                 require_envelope: bool = False) -> None:
        if type(verifier) is not HumanIdentityVerifier:
            raise HumanApprovalError(
        ...
        # Freeze this explicit trust root. A later global set_verifier(), or a
        # call to register_principal() on the retained handle, cannot redirect
        # an already-constructed execution gate to agent-chosen keys.
        verifier.seal()
