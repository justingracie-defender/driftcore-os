...
class SqliteNonceStore:
    ...
    def _configure_database(self, busy_timeout_ms: int) -> None:
        """CLAIM startup-lock-is-bounded: Transient SQLite startup locks are retried
        within the configured wait budget; exhaustion refuses with NonceStoreLocked.

        WAL negotiation can return SQLITE_BUSY immediately even with a busy
        handler installed. All statements below are repeatable initialization;
        no grant or approval is consumed by a retry.
        """
        ...
        while True:
            ...
            try:
                ...
                self._db.execute(
                    ...
                    "nonce TEXT PRIMARY KEY, ts REAL NOT NULL)")
                self._db.execute("CREATE INDEX IF NOT EXISTS nonces_ts ON nonces(ts)")
                self._db.execute(
                ...
                if code is None:
                    # Python 3.10 does not expose sqlite_errorcode. Retry only
                    # SQLite's exact lock messages; unknown errors still refuse.
                    retryable = str(error) in ("database is locked", "database table is locked")
                else:
                    retryable = (code & 0xff) in (5, 6)  # SQLITE_BUSY, SQLITE_LOCKED
                if not retryable:
                    raise
                remaining = max(0.0, deadline - time.monotonic())
                if remaining == 0.0:
                    raise NonceStoreLocked(
                        f"nonce database startup remained locked after the "
                        f"{busy_timeout_ms}ms wait budget") from error
