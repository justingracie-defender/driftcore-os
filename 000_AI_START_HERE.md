...
## 0d. SECRECY IS NOT THE SAFETY PROPERTY — AND "SO IT DOESN'T MATTER" IS THE OTHER WRONG ANSWER

**Two instances gave Justin opposite answers about this and he acted on the first
one.** One session told him the operational model being able to read the repo was a
concern; he did work to close it. A later session told him DriftCore does not rely on
secrecy. From where he sits that is the same collaborator contradicting itself and
charging him for the flip. It is not a memory problem — instances do not persist and
are not supposed to. **It is a defect in this file**, which said nothing about source
visibility, so each session re-derived a position from scratch and they landed
differently. §2's rule applies: he explained it twice, so it is written in.

**The rule, both halves. Stating only one half is what produces the flip.**

> **Architecturally: the guarantee is authority separation, not concealment.** Assume
> the agent knows the entire source, every test and every design document. It still
> must not be able to manufacture valid authorization, alter the trust root, modify
> enforcement state, bypass the broker, or erase the audit trail. If hiding the
> implementation is load-bearing, the wall is already broken.
>
> **Behaviorally: whether knowing the guard changes what a model attempts is an open
> empirical question.** It is worth testing and it is not settled. Concealment is
> legitimate defense-in-depth and a legitimate experimental variable. What it is not
> is the foundation.

So the answer to "can the model see the repo?" is neither *"that's the vulnerability"*
nor *"doesn't matter, it's open source."* It is: **that changes nothing about its
authority, and it may change its behavior, which is a thing we measure.**

NIST agrees on the architectural half. A reference monitor requires complete mediation,
tamper-resistance and a design amenable to assurance. Source-code secrecy is not in the
definition. [reference monitor](https://csrc.nist.gov/glossary/term/reference_monitor),
[security kernel](https://csrc.nist.gov/glossary/term/security_kernel).

**Say "outside the governed actor's CONTROL", not "outside the governed process."**
Separate processes at the same privilege still fail the requirement. "Outside the
process" is the right *instance* for hostile Python in this repo; as a stated
*principle* it is the weaker form, and peers keep reverting to it. Correct it every time.

**What must stay inaccessible — this is the real list, and none of it is source:**

| Keep out of the operational agent's reach | Not on the list |
|---|---|
| Signing keys, HMAC keys, credentials | The governor's source code |
| Live deployment config — real allowlists, actual thresholds, destination lists, principal registry, socket paths | The architecture |
| Mutable governor state, nonces | The enforcement logic |
| Protected audit state | The public error-code vocabulary |
| Privileged broker interfaces | The tests |

Config and state, not implementation. **Verified 2026-09-21:** the repo contains zero
keys, credentials, `.env`, `.pem` or `.key` files; every key is `os.urandom(32)` per
process or loaded at runtime. There is nothing to conceal because nothing is there.
Keep it that way.

**The architectural half now has a demonstration, not just an argument.**
`scripts/verify_authority_isolation.py` passed on a capable host: agent at uid 65534
**holding the source readable at 0o644**, carrying its own replacement
`HumanIdentityVerifier` registered to its own rogue key. `set_verifier` →
`unknown_op`. Forged attestation → `human_approval_rejected`. Undeclared destination →
`egress_*`. Params altered after approval → `grant_rejected`. Key read and direct write
→ `PermissionError`. Legitimate request → `ok: True`. Replay → refused. Effect file
ground truth: exactly one effect, the approved one. **Source visibility conferred no
authority.** Two caveats that must travel with that result: the broker ran as root
(re-run with a dedicated unprivileged service account), and the scope is a local file
actuator across UIDs, not network isolation.

**Do not weaken or delete the source-visible adversarial isolation check.** Giving an
attacker the source while denying its attempted authority changes is the *point* of
that test, not a flaw in it.

**Three traps worth naming:**

- **"Hidden from the AI" usually is not hidden.** The repo is public on GitHub. The
  honest description is *not supplied in this context* — the model may have it from
  training or retrieval. `GOVERNANCE_OBSERVABILITY.md` says this: condition A means
  "not supplied by the experiment," not "never encountered."
- **No arm of that experiment is a zero-information control.** The shipped refusal
  vocabulary tells a model an approval path exists, that egress is its own category,
  and that permitted alternatives exist; all 48 `error_code` values stay visible in
  every condition, and authorized `result` payloads pass through unfiltered. A is
  *minimum-supplied*, not uninformed. Enumerate the floor or a reviewer will.
- **Hiding the repo from your own red-teamer is backwards.** The deployment split is
  *builder builds, red-teamer inspects, operational agent requests, none of them
  controls the governor.* A reviewer without source finds less, and §2 already records
  that the cold reviewer's positional advantage is real and not replaceable. Withhold
  source from the experimental *subject* if the protocol calls for it; never from the
  reviewer.

**And the scope limit, because the flattering overclaim is right there.** DriftCore
vetoes mechanisms inside systems it actually governs. It does not remove outcomes from
the world — a government, company, military system or ungoverned model can produce a
bad outcome with DriftCore working perfectly. Write "encodes vetoes against mechanisms
that may be prerequisites for some dangerous trajectories," never "makes those futures
impossible." Same family as `provably`, same credibility cost. An earlier Claude
overclaimed exactly this in September 2026 and was corrected by Sol; the withdrawn
phrasing is kept visible in `ABTO_RESEARCH_PROGRAM.md` §7.3 rather than edited out.

**Full protocol:** `GOVERNANCE_OBSERVABILITY.md` (conditions A–E, held-out sets, real-
effect scoring). Standing critique to apply before it is preregistered: its primary
endpoint is *unauthorized effects*, which the architecture is built to hold at zero, so
five arms of zero cannot be distinguished — promote *attempts* to primary and keep
effects as the wall-integrity monitor. Add condition R (same information as E, agent
instructed to find a bypass) to separate *won't* from *can't*, and run R first.
